...
export class TranscriptionService {
  ...
  static async transcribeAudio(
    ...
    try {
      ...
      callbacks?.onProgressLog?.(`[audio] ${processedBlob.size} Bytes PCM WAV (16.000 Hz, mono)`);
    } catch (err) {
      console.warn('Browser WAV conversion error, falling back to original blob:', err);
      finalMime = mimeType;
      callbacks?.onProgressLog?.(`[audio] ${audioBlob.size} Bytes (${mimeType})`);
    }
...
[
  {
    ...
    try {
      ...
      if (errMsg.includes('404') || errMsg.includes('No endpoints found')) {
        callbacks?.onProgressLog?.(`[retry] Modell ${audioModel} nicht verfügbar, wechsel auf google/gemini-3.8-flash...`);
        audioModel = 'google/gemini-3.8-flash';
        response = await client.chatCompletion({
          prompt,
          model: audioModel,
